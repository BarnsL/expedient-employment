...
class PipelineTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        """Load the deterministic fixture profile and job fixtures.

        Scoring/agent tests pin tests/fixtures/sample_profile.json (the shipped
        demo profile) so edits to the owner's live config/profile.json never
        change deterministic test expectations.
        """
        self.profile = json.loads((ROOT / "tests" / "fixtures" / "sample_profile.json").read_text(encoding="utf-8"))
    ...
    def test_agent_web_browser_uses_authenticated_read_only_routes(self) -> None:
        ...
        def transport(method, path, body, headers):
            ...
            if path == "/health":
                return {"ok": True, "app": "Agent Web Browser", "port": 7896}
            if path == "/status":
                return {
                    "ok": True,
                    "activeTab": 7,

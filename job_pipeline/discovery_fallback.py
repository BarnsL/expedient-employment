...
def _login_required_hint(board: str | None) -> str:
    """Return a user-facing login handoff hint for session-gated boards."""
    if not board or board not in session_sites(load_policy()):
        return ""
    return (
        f" login_required: sign in through the AWB window with "
        f"scripts\\request-site-login.ps1 -Site {board}, then re-run discovery."
    )
DIRECT_ATS_DOMAINS = (
    "greenhouse.io",
    "lever.co",
...
def resolve_employer_application(
    ...
    raise WebClawError(f"Could not resolve {source_url}: {detail}{_login_required_hint(board)}")

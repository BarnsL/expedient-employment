...
Re-import replaces the protected record only after a successful write. If no replacement is available, the prior encrypted key remains visible and clearable. Clear does not claim success when deletion fails. A successful credential change restarts only the owned control service. Job-pipeline subprocesses do not inherit the control provider key, and provider output is credential-redacted before tool execution or transcript storage. Users sharing one Windows account must clear the saved key before handing over the account.
